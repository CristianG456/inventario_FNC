...
<head>
    ...
    <style>
        ...
        #sidebar { width: var(--sidebar-width); min-height: 100vh; background: var(--sidebar-bg); position: fixed; top: 0; left: 0; z-index: 100; transition: transform .3s ease; }
        #sidebar .nav-link { color: rgba(255,255,255,.75); border-radius: 6px; margin: 2px 10px; padding: 9px 14px; transition: background .2s, color .2s; }
        #sidebar .nav-link:hover, #sidebar .nav-link.active { background: var(--sidebar-hover); color: #fff; }
        #sidebar .nav-link i { width: 22px; }
        .sidebar-brand { padding: 20px 20px 10px; color: #fff; font-weight: 700; font-size: 1.1rem; border-bottom: 1px solid rgba(255,255,255,.15); margin-bottom: 10px; }
        .main-content { margin-left: var(--sidebar-width); padding: 0; }
        .topbar { background: #fff; border-bottom: 1px solid #dee2e6; padding: 12px 24px; position: sticky; top: 0; z-index: 99; }
        .content-area { padding: 24px; }
        .card { border: none; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,.07); }
        .table thead th { background: var(--sidebar-bg); color: #fff; font-weight: 600; border: none; font-size: .85rem; text-transform: uppercase; letter-spacing: .03em; }
...
<nav id="sidebar">
    <div class="sidebar-brand d-flex align-items-center gap-2">
        <img src="{{ asset('imagenes/federacion cafeteros logo.png') }}" alt="Federación Nacional de Cafeteros" width="10" height="10" style="object-fit:contain; border-radius:4px; background:#fff; padding:1px;">
